...
var PrintContext = (0, import_preact4.createContext)(null);
var PrintPage = ({ pageResult }) => {
  const [imageUrl, setImageUrl] = (0, import_hooks2.useState)("");
...
function PrintProvider({ children }) {
  ...
  const iframeRef = (0, import_hooks2.useRef)(null);
  const [progress, setProgress] = (0, import_hooks2.useState)(null);
  const [isReady, setIsReady] = (0, import_hooks2.useState)(false);
  const [isPrinting, setIsPrinting] = (0, import_hooks2.useState)(false);
  const [pages, setPages] = (0, import_hooks2.useState)([]);
  ...
  (0, import_hooks2.useEffect)(() => {
    ...
    if (mountNode && pages.length > 0) {
      (0, import_preact4.render)(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PrintLayout, { pages }), mountNode);
      setIsReady(true);
      return () => {
        if (mountNode) {
          (0, import_preact4.render)(null, mountNode);
        }
      };
    }
  }, [pages]);
...
function usePrintContext() {
  const context = (0, import_hooks2.useContext)(PrintContext);
  if (!context) {
