...
var UIComponent = class {
  ...
  sortChildren() {
    this.children.sort((a, b2) => a.priority - b2.priority);
  }
  removeChild(child) {
...
var uiReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case UI_UPDATE_COMPONENT_STATE: {
      ...
      const filteredPatch = Object.fromEntries(Object.entries(patch).filter(([k2]) => k2 in current));
...
function resolveMenuItem(item, state) {
  const dyn = (v2) => typeof v2 === "function" ? v2(state) : v2;
  if (item.type === "group") {
    return {
      ...item,
