...
var SpreadPlugin = class extends w2 {
  constructor(id, registry, cfg) {
    super(id, registry);
    this.spreadEmitter$ = E22();
    this.resetReady();
    this.dispatch(setSpreadMode(
      cfg.defaultSpreadMode ?? "none"
      /* None */
    ));
    const loaderPlugin = registry.getPlugin("loader");
    loaderPlugin.provides().onDocumentLoaded((document2) => this.documentLoaded(document2));
  }
  ...
  documentLoaded(document2) {
    this.dispatchCoreAction(Q2(this.getSpreadPagesObjects(document2.pages)));
    this.markReady();
  }
    ...
    switch (this.state.spreadMode) {
      ...
      case "odd":
        return Array.from(
          ...
          (_22, i2) => pages.slice(i2 * 2, i2 * 2 + 2)
        );
      case "even":
        return [
          [pages[0]],
          ...Array.from(
            ...
            (_22, i2) => pages.slice(1 + i2 * 2, 1 + i2 * 2 + 2)
          )
        ];
      default:
        return pages.map((page) => [page]);
...
var useSpreadPlugin = () => ee2(SpreadPlugin.id);
var useSpreadCapability = () => ae(SpreadPlugin.id);
var useSpread = () => {
  const { provides: spreadProvider } = useSpreadCapability();
  const [spreadMode, setSpreadMode2] = (0, import_react2.useState)(SpreadMode.None);
  (0, import_react2.useEffect)(() => {
    return spreadProvider?.onSpreadChange((spreadMode2) => {
      setSpreadMode2(spreadMode2);
    });
  }, [spreadProvider]);
  return {
