const v = "LOAD_DOCUMENT", F = "SET_DOCUMENT", D = () => ({ type: v }), R = (r) => ({
  type: F,
  payload: r
});
var $ = "­", E = "​", C = "⁠", O = "\uFEFF", L = "￾", I = "￿", P = Object.freeze([
  $,
  E,
  C,
  O,
...
function g(r, e, t) {
  ...
  if (s === "object") {
    ...
    if (t.has(o))
      return !0;
    t.add(o);
    const n = Array.isArray(r), l = Array.isArray(e);
    return n && l ? k(r, e, t) : !n && !l ? U(r, e, t) : !1;
  }
  return !1;
}
function T(r, e) {
...
function m(r) {
  return b.has(r) || b.set(r, ++H), b.get(r);
}
function k(r, e, t) {
...
function U(r, e, t) {
  const s = Object.keys(r).sort(), o = Object.keys(e).sort();
...
class j {
  constructor(e, t) {
    ...
    this.coreStore = this.registry.getStore(), this.pluginStore = this.coreStore.getPluginStore(this.id), this.unsubscribeFromState = this.pluginStore.subscribeToState((s, o, n) => {
      this.onStoreUpdated(n, o);
    }), this.unsubscribeFromCoreStore = this.coreStore.subscribe((s, o, n) => {
      this.onCoreStoreUpdated(n, o);
  ...
  debouncedDispatch(e, t = 100) {
    const s = Date.now(), o = this.debouncedActions[e.type] || 0;
    return s - o >= t ? (this.debouncedActions[e.type] = s, this.dispatch(e), !0) : !1;
  }
  /**
...
class M {
  constructor(e, t) {
    this.handler = e, this.options = t, this.lastRun = 0, this.handle = (s) => {
  ...
  throttle(e) {
    ...
    const t = Date.now(), s = this.options.throttleMode || "leading-trailing";
    t - this.lastRun >= this.options.wait && (s === "leading-trailing" && this.handler(e), this.lastRun = t), this.timeoutId && window.clearTimeout(this.timeoutId), this.timeoutId = window.setTimeout(
      () => {
...
function z() {
  ...
  return {
    emit: (e = void 0) => r.forEach((t) => t(e)),
    on: (e) => (r.add(e), () => r.delete(e)),
    off: (e) => r.delete(e),
    clear: () => r.clear()
  };
}
function S(r, e = g) {
  const t = /* @__PURE__ */ new Set(), s = /* @__PURE__ */ new Map();
  let o = r;
  const n = (i) => t.forEach((a) => a(i)), l = (i, a) => {
    let d = i, u = () => {
    };
    if (a) {
      const c = new M(i, a);
      d = c.handle, u = () => c.destroy(), s.set(i, { wrapped: d, destroy: u });
    }
    return o !== void 0 && d(o), t.add(d), () => {
      t.delete(d), u(), s.delete(i);
    };
  };
  return {
    /* emitter behaviour ---------------------------------------- */
    ...
    emit(i = void 0) {
      (o === void 0 || !e(o, i)) && (o = i, n(i));
    },
    on: l,
    off(i) {
      const a = s.get(i);
      a ? (t.delete(a.wrapped), a.destroy(), s.delete(i)) : t.delete(i);
    },
    clear() {
      t.clear(), s.forEach((i) => i.destroy()), s.clear();
    },
    /* derived hook --------------------------------------------- */
    select(i, a = g) {
      return (d, u) => {
...
class N {
  async load(e) {
    const { pdfFile: t, options: s, engine: o } = e, n = o.openDocumentUrl(t, s);
    return new Promise((l, i) => {
      n.wait(
        ...
        // Error callback
        (a) => {
          a.type === "abort" ? i(new Error(`PDF loading aborted: ${a.reason}`)) : i(new Error(`PDF loading failed: ${a.reason}`));
        }
      );
    });
  }
...
class _ {
  async load(e) {
    const { pdfFile: t, options: s, engine: o } = e, n = o.openDocumentFromBuffer(t, (s == null ? void 0 : s.password) || "");
    return new Promise((l, i) => {
      n.wait(
        // Success callback
        (a) => l(a),
...
class x {
  constructor() {
    ...
    const e = new _();
    this.registerStrategy("url", new N()), this.registerStrategy("buffer", e), this.addStrategyResolver((t) => q(t) ? this.strategies.get("url") : this.strategies.get("buffer"));
  }
...
const y = class y extends j {
  constructor(e, t, s) {
    super(e, t), this.id = e, this.engine = s, this.loaderHandlers$ = S(), this.documentLoadedHandlers$ = S(), this.openFileRequest$ = z(), this.documentLoader = new x();
  }
  buildCapability() {
    return {
  ...
  async initialize(e) {
    e.defaultStrategies && Object.entries(e.defaultStrategies).forEach(([t, s]) => {
      this.documentLoader.registerStrategy(t, s);
  ...
  async loadDocument(e) {
    try {
      this.loaderHandlers$.emit({ type: "start", documentId: e.pdfFile.id }), this.dispatchCoreAction(D());
      const t = await this.documentLoader.loadDocument({
        ...e,
        engine: this.engine
      });
      return this.dispatchCoreAction(R(t)), this.loadedDocument = t, this.loaderHandlers$.emit({ type: "complete", documentId: e.pdfFile.id }), this.documentLoadedHandlers$.emit(t), t;
    } catch (t) {
      const s = {
        type: "error",
      ...
      throw this.loaderHandlers$.emit(s), t;
    }
  }
  async destroy() {
...
//# sourceMappingURL=loader-plugin-DC0zA4Bl.js.map
