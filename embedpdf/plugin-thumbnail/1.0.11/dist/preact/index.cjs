...
function ThumbnailsPane({
  ...
  const viewportRef = (0, import_hooks2.useRef)(null);
  const [window2, setWindow] = (0, import_hooks2.useState)(null);
  (0, import_hooks2.useEffect)(() => thumbs?.onWindow(setWindow), [thumbs]);
...
function ThumbImg({ meta, style, ...props }) {
  const { provides: thumbs } = useThumbnailCapability();
  const [url, setUrl] = (0, import_hooks4.useState)();
  const urlRef = (0, import_hooks4.useRef)(null);
